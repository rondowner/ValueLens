...
window.ValueLensViewport=function({container,stage,canvas,onTap,onChange}){
 const MIN=.1,MAX=8,SLOP=7;let scale=1,x=0,y=0,start=null,moved=false,pinch=null,interactionEnabled=true;const pointers=new Map();
 ...
 container.addEventListener("pointerdown",e=>{if(!interactionEnabled)return;e.preventDefault();container.setPointerCapture(e.pointerId);pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});start={x:e.clientX,y:e.clientY,panX:x,panY:y};moved=false;if(pointers.size===2){const p=[...pointers.values()];pinch={d:Math.hypot(p[1].x-p[0].x,p[1].y-p[0].y),scale};}container.classList.add("is-panning");});
 ...
 function end(e){if(!interactionEnabled)return;const tap=!moved&&pointers.size===1;pointers.delete(e.pointerId);container.classList.remove("is-panning");if(tap)onTap?.(imagePoint(e.clientX,e.clientY));start=null;pinch=null;}
 ...
 return{fit,actual:()=>setScale(1),zoomIn:()=>setScale(scale*1.25),zoomOut:()=>setScale(scale/1.25),getScale:()=>scale,imagePoint,setInteractionEnabled};};
